...
A **Collective** is a new approach for like-minded creators to monetize the efforts of group cooperation. A collective is a group of social tokens that have bonded their directional success to a base token. Collectives allow participants to pool resources with their network of social tokens. Collectives reorient directional goals towards the success of groups rather than individuals alone. 

Collectives allow communities and fans to contribute to the success of ideas, groups, or categories. Viewed through this lens, collectives can be seen as an index for ideas. Collectives are the beginning of an intersection between creators and communities working together for a common purpose.

Strata gives you the ability to create collectives that require permission to join, or ones that are permissionless. The **OPEN Collective (OPEN)** is the default collective on Strata. When a token is created without a collective or base mint specified, that token will be placed into the Open Collective. The Open Collective is permissionless, thus allows anyone to join. It also takes no fees or royalties. The Open Collective stands as our model for a truly decentralized collective. We, the Strata Team, take no royalties from the OPEN token, as we believe doing so would stifle innovation.
